 ...
 */
@Component
//spring boot项目可以直接被mybatis检测到并加载此插件，spring项目需要配置到mybatis-config.xml中
@Profile({"dev", "test"}) //只在开发和测试环境开启，生产就算了吧
...
public class SqlCostInterceptor implements Interceptor {
     ...
     * 处理参数为List的场景
     */
    private String handleListParameter(String sql, Collection<?> col, List<ParameterMapping> parameterMappingList) throws IllegalAccessException, IntrospectionException, InvocationTargetException {
        if (!CollectionUtils.isEmpty(col)) {
            for (Object obj : col) {
                sql = this.formatSql(sql, obj, parameterMappingList);
            }
        }
    ...
    private String handleMapParameter(String sql, Map<?, ?> paramMap, List<ParameterMapping> parameterMappingList) {
        ...
        for (ParameterMapping parameterMapping : parameterMappingList) {
            ...
            Object propertyValue = currentMap.get(propertyName);
    ...
    private String handleCommonParameter(String sql, List<ParameterMapping> parameterMappingList, Class<?> parameterObjectClass,
        ...
        if (isPrimitiveOrPrimitiveWrapper(parameterObjectClass)) {
            ...
            for (ParameterMapping parameterMapping : parameterMappingList) {
                final String fieldName = parameterMapping.getProperty();
